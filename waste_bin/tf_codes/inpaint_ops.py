...
def contextual_attention(f, b, mask=None, ksize=3, stride=1, rate=1,
    ...
    raw_w = tf.extract_image_patches(
        b, ksizes=[1, kernel, kernel, 1], strides=[1, rate*stride, rate*stride, 1], rates=[1,1,1,1], padding='SAME'
    )  # B, H//(d*s), W//(d*s), 2d*2d*C
    raw_w = tf.reshape(raw_w, [raw_int_bs[0], -1, kernel, kernel, raw_int_bs[3]])  # B, H//(d*s) * W//(d*s), k, k, C
    raw_w = tf.transpose(raw_w, [0, 2, 3, 4, 1])  # transpose to b*k*k*c*hw --> B, k, k, C, H//(d*s) * W//(d*s)
    # downscaling foreground option: downscaling both foreground and
    # background for matching and use original background for reconstruction.
    f = resize(f, scale=1./rate, func=tf.image.resize_nearest_neighbor)  # B, H//d, W//d, C
    b = resize(b, to_shape=[int(raw_int_bs[1]/rate), int(raw_int_bs[2]/rate)], func=tf.image.resize_nearest_neighbor)  # https://github.com/tensorflow/tensorflow/issues/11651
    # B, H//d, W//d, C
    if mask is not None:
        mask = resize(mask, scale=1./rate, func=tf.image.resize_nearest_neighbor)  # B, H//d, W//d, C
    fs = tf.shape(f)
    int_fs = f.get_shape().as_list()  # B, H//d, W//d, C
    f_groups = tf.split(f, int_fs[0], axis=0)  # list[Tensor(H//d, W//d, C)] len: B
    ...
    w = tf.reshape(w, [int_fs[0], -1, ksize, ksize, int_fs[3]])  # B, H//d * W//d, k, k, C
    ...
    w_groups = tf.split(w, int_bs[0], axis=0)  # list[Tensor(1, k, k, C, H//d * W//d)] len: B
    raw_w_groups = tf.split(raw_w, int_bs[0], axis=0)  # list[Tensor(2d, 2d, C, H//d * W//d)] len: B
    y = []
    offsets = []
    k = fuse_k
    scale = softmax_scale
    ...
    for xi, wi, raw_wi in zip(f_groups, w_groups, raw_w_groups):
        ...
        yi = tf.nn.conv2d(xi, wi_normed, strides=[1, 1, 1, 1], padding="SAME")  # 1, H//d, W//d, H//d * W//d
        # ???: xi is not normalized ???

        # conv implementation for fuse scores to encourage large patches
        if fuse:
            yi = tf.reshape(yi, [1, fs[1]*fs[2], bs[1]*bs[2], 1])  # 1, H//d * W//d, H//d * W//d, 1
            yi = tf.nn.conv2d(yi, fuse_weight, strides=[1,1,1,1], padding='SAME')  # 1, H//d * W//d, H//d * W//d, 1
            yi = tf.reshape(yi, [1, fs[1], fs[2], bs[1], bs[2]])  # 1, H//d, W//d, H//d, W//d
            yi = tf.transpose(yi, [0, 2, 1, 4, 3])  # 1, W//d, H//d, W//d, H//d
            yi = tf.reshape(yi, [1, fs[1]*fs[2], bs[1]*bs[2], 1])  # 1, W//d * H//d, W//d * H//d, 1
            yi = tf.nn.conv2d(yi, fuse_weight, strides=[1,1,1,1], padding='SAME')  # 1, W//d * H//d, W//d * H//d, 1
            yi = tf.reshape(yi, [1, fs[2], fs[1], bs[2], bs[1]])  # 1, W//d * H//d, W//d * H//d, 1
        ...
        offset = tf.stack([offset // fs[2], offset % fs[2]], axis=-1)  # this is the most correlated idx
